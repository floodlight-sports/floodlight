...
def get_column_names_from_csv(filepath_data: Union[str, Path], delimiter = ",") -> List[str]:
    ...
    delimiter: str
        Delimiter specified in Kinexon .csv-file.
    ...
    # Delete last entry in list if it is a newline
    if columns[-1] == "\n":
        del columns[-1]
...
def _get_column_links(filepath_data: Union[str, Path], delimiter = ",") -> Union[None, Dict[str, int]]:
    ...
    for i in range(1, len(recorded_columns),necessary_min_length_steps): 
        ...
        for key in mapping:
            # create links
            if key in recorded_columns_tmp:
                column_links.update({mapping[key]: recorded_columns_tmp.index(key)})
...
def read_position_data_csv(filepath_data: Union[str, Path], delimiter=",") -> List[XY]:
    ...
    with open(str(filepath_data), "r", encoding="utf-8") as f:
        ...
        while True:
            ...
            # split str
            line = line_string.split(delimiter)
            # Extract timestamp at fixed index 0
            ...
            for i in range(1, len(line),number_of_unique_column_links):
                ...
                if group_id == "":
                    continue
                # set column
                x_col = links[group_id][line_tmp[column_links[identifier]]] * 2
                y_col = x_col + 1
                # set row
                row = int((timestamp - t_null) / (1000 / framerate))
                # set (x, y)-data
                x_coordinate = line_tmp[column_links["x_coord"]]
                y_coordinate = line_tmp[column_links["y_coord"]]
